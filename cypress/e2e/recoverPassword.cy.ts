describe('TableRise :: Recover Password', () => {
    context('When the user recover the password', () => {
        beforeEach(() => {
            ...
            cy.intercept('GET', `**/verify?email=fake%40email.com&flow=update-password`, {
                statusCode: 200,
            }).as('verifyEmail');

            cy.intercept('PATCH', `**/update/password?email=fake%40email.com`, {
                statusCode: 200,
            }).as('updatePassword');
        });
    ...
    context('When the user recover the password - error when checking email', () => {
        ...
        it('If there is a server error', () => {
            ...
            cy.get('.form-span').should('be.visible').and('contain', 'Erro no servidor');
        });
    });
    ...
    context(
        'When the user recover the password - error when checking secret question',
        () => {
            beforeEach(() => {
                cy.visit('/password-recover');
    ...
    context('When the user recover the password - error when update password', () => {
        ...
        it('User status invalid', () => {
            ...
            cy.get('.form-span')
                ...
                .and(
                    'contain',
                    'O status do usuário é inválido para realizar esta operação'
                );
        });
