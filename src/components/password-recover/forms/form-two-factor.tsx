...
export default function FormTwoFactor() {
    ...
    const handleKeyPress = (e: React.KeyboardEvent<HTMLInputElement>, index: number) => {
        ...
        if ((e.key === 'Backspace' || e.key === 'Delete') && inputElement.value === '') {
            const previousField = document.getElementById(`fild${index - 1}`) as HTMLInputElement | null;
            if (previousField) {
                previousField.focus();
            }
        } else if (inputElement.value.length === 1 && e.key !== 'Backspace' && e.key !== 'Delete') {
    ...
    const nextInput = (e: React.FormEvent<HTMLInputElement>, index: number) => {
        ...
        if (value.length === 1) {
            const nextField = document.getElementById(`fild${index + 1}`) as HTMLInputElement | null;
                ...
                <div className="flex gap-2">
                    {
                        num6.map((_, index) => (
                            <input
                                {...register(`fild${index}` as `fild${0 | 1 | 2 | 3 | 4 | 5}`)}
                                id={`fild${index}`}
                                key={index}
                                type="text"
                                inputMode="numeric"
