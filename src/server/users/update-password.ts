...
export const authenticateEmail = async (email: string, code: string) => {
    try {
        const { data }: AxiosResponse<Response> = await apiUser.patch('/authenticate/email', null, {
...
export const authenticate2fa = async (email: string, code: string) => {
    try {
        const response: AxiosResponse<Response> = await apiUser.patch('/authenticate/2fa', null, {
...
export const authenticateSecretQuestion = async (email: string, question: string, answer: string) => {
    try {
        const { data }: AxiosResponse<Response> = await apiUser.patch('/authenticate/secret-question',
            { question, answer },
            {
                params: { email, flow: 'update-password' },
            });
...
export const sendNewPassword = async (email: string, newPassword: string) => {
    try {
        await apiUser.patch('/update/password',
            {
